# -*- coding: utf-8 -*-
"""
APSOLUTNA_VISINA - Ocitavanje nadmorske visine na tocki (QGIS skripta)
=======================================================================
Za svaki od 8 tockastih slojeva:
  1. Uzorkuje vrijednost rastera "nadmorska_visina" na lokaciji tocke
  2. Dodaje atribut "elev" u sloj
  3. Izvozi CSV  →  {OUTPUT_DIR}/{naziv_sloja}.csv  (fid, elev)

Uz to izvozi pozadinski histogram:
  4. background_elev.csv  →  elev_m, n_piksela, postotak
...
OUTPUT_DIR = r"C:\Users\Martin\Desktop\skripte_za_diplomski\deskriptivna\apsolutna_visina_\csv_output"
...
def export_background(raster_layer):
    """
    Izvozi histogram nadmorske visine iz cijelog rastera.
    Koristi QgsRasterBandStats za min/max/mean/sd,
    zatim provider.histogram() za N_HIST_BINS stupaca.
    """
    ...
    elev_min = st.minimumValue
    elev_max = st.maximumValue
    print(f"  Min={elev_min:.1f}m  Max={elev_max:.1f}m  Mean={st.mean:.1f}m  SD={st.stdDev:.1f}m")

    print(f"  Racunam histogram ({N_HIST_BINS} stupaca)...")
    hist      = provider.histogram(1, N_HIST_BINS, elev_min, elev_max)
    hist_vec  = hist.histogramVector
    bin_width = (elev_max - elev_min) / N_HIST_BINS
    total     = sum(hist_vec)

    out_path = os.path.join(OUTPUT_DIR, "background_elev.csv")
    with open(out_path, "w", newline="", encoding="utf-8-sig") as fh:
        w = csv.writer(fh)
        w.writerow(["elev_m", "n_piksela", "postotak"])
        for i, count in enumerate(hist_vec):
            bin_center = elev_min + (i + 0.5) * bin_width
            w.writerow([round(bin_center, 2), int(count), round(count / total * 100, 6)])

    print(f"  Exportano: background_elev.csv  ({N_HIST_BINS} stupaca, {total:,} piksela)")
...
def run_all():
    print("=" * 60)
    print("APSOLUTNA VISINA - početak analize")
    print("=" * 60)
