# -*- coding: utf-8 -*-
"""
RES_APSOLUTNA_VISINA - Deskriptivna statistika i pozadinska usporedba
======================================================================
Ulaz:  8 CSV-a iz apsolutna_visina.py  (fid, elev)
       + background_elev.csv           (elev_m, n_piksela, postotak)
       mapa: deskriptivna/apsolutna_visina_/csv_output/

Izlaz u deskriptivna/res_output/apsolutna_visina/:
  ...
  background_elev_hist.png   - histogram pozadinskog rastera
  comparison_elev.png        - box plot: pozadina + svih 8 slojeva

Pokretanje: python res_apsolutna_visina.py
"""
...
INPUT_DIR  = r"C:\Users\Martin\Desktop\skripte_za_diplomski\deskriptivna\apsolutna_visina_\csv_output"
OUTPUT_DIR = r"C:\Users\Martin\Desktop\skripte_za_diplomski\deskriptivna\res_output\apsolutna_visina"
...
def load_background():
    bg_path = os.path.join(INPUT_DIR, "background_elev.csv")
    if not os.path.exists(bg_path):
        print("  NEDOSTAJE: background_elev.csv — pokreni apsolutna_visina.py u QGIS-u")
        return None
...
def weighted_stats_bg(bg_df):
    ...
    vals  = bg_df["elev_m"].values.astype(float)
    ...
    return {
        ...
        "whislo": round(max(float(vals.min()), q1 - 1.5 * iqr), 2),
        "whishi": round(min(float(vals.max()), q3 + 1.5 * iqr), 2),
    }
...
def plot_boxplot(data, layers, out_path, title=""):
    ...
    ax.set_ylabel("Nadmorska visina (m)")
...
def plot_background_hist(bg_df, out_path):
    """Histogram (fill_between) pozadinskog rastera nadmorske visine."""
    s = weighted_stats_bg(bg_df)

    fig, ax = plt.subplots(figsize=(8, 5))
    ax.fill_between(bg_df["elev_m"], bg_df["postotak"],
                    alpha=0.65, color="#2E75B6", linewidth=0)
    ax.plot(bg_df["elev_m"], bg_df["postotak"],
    ...
    ax.set_title("Pozadinska distribucija — nadmorska visina (cijeli raster)", fontsize=11)
    ax.set_xlabel("Nadmorska visina (m)")
...
def plot_comparison(data, bg_df, out_path):
    ...
    ax.set_ylabel("Nadmorska visina (m)")
    ax.set_title("Nadmorska visina — pozadina vs. slojevi", fontsize=11, pad=10)
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    plt.tight_layout()
    fig.savefig(out_path, dpi=150, bbox_inches="tight")
...
def run():
    ...
    print("=" * 60)
    print("APSOLUTNA VISINA - učitavanje CSV-a")
    print("=" * 60)
    data  = load_all()
    bg_df = load_background()

    if not data:
        print("Nema podataka — provjeri INPUT_DIR i pokreni apsolutna_visina.py u QGIS-u.")
        return
    ...
    summary_df.to_csv(os.path.join(OUTPUT_DIR, "summary_stats.csv"), index=False, encoding="utf-8-sig")
    print(f"\n  → summary_stats.csv")
    ...
    if bg_df is not None:
        ...
        print(f"  Pozadina — mean={s_bg['mean']} m  median={s_bg['median']} m  "
              f"Q1={s_bg['q1']}  Q3={s_bg['q3']}  IQR={s_bg['iqr']}")

        plot_background_hist(bg_df, os.path.join(OUTPUT_DIR, "background_elev_hist.png"))
        print("  background_elev_hist.png")

        plot_comparison(data, bg_df, os.path.join(OUTPUT_DIR, "comparison_elev.png"))
        print("  comparison_elev.png")
    else:
        print("\n  Pozadinska usporedba preskočena (nema background_elev.csv)")
